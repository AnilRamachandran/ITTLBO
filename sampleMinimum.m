function [ samples hessians] = sampleMinimum(nSamples, Xsamples, Ysamples, sigma0, sigma, l, xmin, xmax, nFeatures)
...
%		noise = load('../RmoduleFinal/auxiliaryFiles/noise.txt');
		...
		if (size(Xsamples, 1) > 0)
			...
			if (size(Xsamples, 1) < nFeatures)
                ...
                inverseWoodbury = invChol_mex(woodbury);
                ...
                Sigma = invChol_mex(cross(tDesignMatrix, tDesignMatrix') / sigma0(i) + eye(nFeatures));
		...
		[ sample hessian ]= globalOptimization(targetVector, targetVectorGradient, xmin, xmax, zeros(0, d));
