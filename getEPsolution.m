function [ ret ] = getEPsolution(X, y, m, l, sigma, sigma0, hessian)
    ...
    vAux_temp = diag([ zeros(sizeB, 1) ; retEP.vHat.^-1 ]) + K;  
    vAux_temp = topdm_vAux(vAux_temp);
    try
        vAux = invChol_mex(vAux_temp);
    catch
        vAux = vAux_temp \ eye(size(vAux_temp));
        flag=1;
    end
	...
	if size(X, 1) == 0
        ...
        KposteriorInv = invChol_mex(computeKmm(X, l, sigma, sigma0));
	end
