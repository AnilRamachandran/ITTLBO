function [ fHat ] = runEPinternal(ymin, d, Kagivenb, magivenb, sigma0)
    ...
    KagivenbInverse = invChol_mex(Kagivenb);
	...
	while (~convergence)
        ...
        Vnew_temp = diag(fHat.vHat) + KagivenbInverse;
        Vnew_temp = topdm(Vnew_temp);
        try
            Vnew = invChol_mex(Vnew_temp);
        catch
            Vnew = Vnew_temp \ eye(size(Vnew_temp));
            flag1=1;
        end
