function [sol, fvals, it,timex] = LBFGS(fg, x0, opt, lmax,scale, epsilon, max_iter, report_interval)
%
%
% S. Ulbrich, May 2002
% Modified by Truyen, Oct 2004 to turn into a MAXIMIMISATION with ROW VECTOR input
...
while it < max_iter
    ...
    [xn,fn,gn] = wolfe(xj,s,stg,fg,opt,f,g,del,theta,scale);
    ...
    it = it + 1;
    timex(it) = toc(starttime);

    fvals(it) = -f;
    %sols(it,:) = xj';

    if timex(it) > prev_time+report_interval
    ...
    if it > 1 & fvals(it-1) ~= 0 & abs(fvals(it)/fvals(it-1) -1) < epsilon
...
%----------------------------------------------------------------
function [best_x,best_f,best_g] = wolfe(xj,s,stg,fct,opt,f,g,del,theta,scale)
%
%
% S. Ulbrich, May 2002
% Modified:
